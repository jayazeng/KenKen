...
public class AdvancedBackTrack2 extends SimpleBackTrack {
	...
	ArrayList<Hashtable<String, ArrayList<Object>>> arrayDomains;

	int n; // size

	// Constructor that takes InputFile
	public AdvancedBackTrack2(InputFile file) {
		super(file);
...
//		tempDomain = cageDomain;  // creates hashtable to store temporary cageDomains

		arrayDomains = new ArrayList<Hashtable<String, ArrayList<Object>>>(n * n);

		arrayDomains.add(cageDomain);
	...
	public boolean cellReduction(String coord, Hashtable<String, ArrayList<Object>> table) {
	...
	public Hashtable<String, ArrayList<Object>> forwardCheck(String coord, Hashtable<String, ArrayList<Object>> table) {

		Hashtable<String, ArrayList<Object>> tempDomain = new Hashtable<String, ArrayList<Object>>();

		tempDomain.putAll(table);
...
//			System.out.println("Depth is "+tree.getDepth());
			System.out.println(cageDomain.values());
		}
	...
	public int advBacktrack() {
		...
		arrayDomains.add(depth, forwardCheck(coor, cageDomain));

//		ArrayList<Object> values = cageDomain.get(coor);  // old version...DELETE WHEN WORKING

		ArrayList<Object> values = (arrayDomains.get(depth).get(coor));

		System.out.println("Depth is " + depth);

		int test = 1;

		while (test < values.size()) {
	...
	public int advBacktrack2() {
		String coor = createCoord(getX(currentNode), getY(currentNode));
//		tempDomain = cageDomain;
	...
	/*
	 * // Janelle's ForwardCheck // method uses the cageDomain and tmp variables to
	 * test if a testValue placed in a certain cell // will cause another cell
	 * further in the puzzle to not have a value // if it does, then the method
	 * returns false and if it doesn't the method returns true
	 * 
	 * @SuppressWarnings("unchecked") public boolean forwardCheck2(int testValue) {
	 * 
	 * 
	 * // create a temporary array lists to list all possible values that can be
	 * updated in the method only ArrayList<ArrayList<Integer>> temp = new
	 * ArrayList<ArrayList<Integer>>(); for (Object ob:cageDomain.values()) {
	 * temp.add((ArrayList<Integer>) ob); }
	 * 
	 * // traverse through the created search tree and add the previous values if
	 * there are any Node traverse = tree.getRoot();
	 * 
	 * // set index int index = 0;
	 * 
	 * 
	 * while (traverse != currentNode) { temp.set(index,
	 * traverse.getLastChild().getValue()); //why are you changing the temp
	 * cageDomain which has less possible numbers? traverse =
	 * traverse.getLastChild(); index++; }
	 * 
	 * // this will get the cell number of the cell we are looking at int cellNum =
	 * this.tree.getDepthOfNode(currentNode) + 1;
	 * 
	 * // set the testValue in the temp list and correct index, but since it's an
	 * arraylist and starts at 0, subtract 1 from the cellNum temp.set(cellNum-1,
	 * testValue);
	 * 
	 * // now the temp array should contain single values to represent the cell
	 * values already chosen // and list of possible values for cells not
	 * initialized yet
	 * 
	 * 
	 * 
	 * // go through and remove the possible values based on row // can use the
	 * subList(int fromIndex, int toIndex) to get just the row int rowNum = (int)
	 * Math.floor(cellNum/n); int rowStart = rowNum * n + 1; for (int i = rowStart;
	 * i < rowStart + n; i++) { temp.get(i).remove(testValue); if
	 * (temp.get(i).isEmpty()) { return false; } }
	 * 
	 * 
	 * // go through and remove the possible values based on column // use the fact
	 * that the col index should be a difference of n from the index
	 * 
	 * int colStart = cellNum - rowStart + 1; for (int i = colStart; i < n*n; i +=
	 * n) { temp.get(i).remove(testValue); if (temp.get(i).isEmpty()) { return
	 * false; } }
	 * 
	 * // no need to go through and remove the possible values based on operation
	 * 
	 * 
	 * // check to see if any list is empty, and if it is return false // else
	 * return true
	 * 
	 * return true; }
	 */
